...
class ClientOrderCreateRequest extends BaseRequest
{
     ...
     */
    public function __construct(protected ClientServiceContract $clientService, protected TimezoneContract $timezoneContract)
    {
        parent::__construct();
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'time' => ['required', 'array'],
            'time.create_time' => ['required', 'date', 'date_format:Y-m-d H:i:s,Y-m-d H:i'],
            'time.time' => ['required', 'required', 'date', 'date_format:Y-m-d H:i:s,Y-m-d H:i'],
            ...
            'payment.card' => [
                'required_if:payment_type,3exists:payment_types,payment_type_id',
            ],
    ...
    public function moreValidation(Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            if (($this->time['time'] !== $this->time['create_time']) && $this->clientService->getPreorderLimit(get_user_id())) {
                $validator->errors()->add('time.time', 'Your limit');
            }
        });
    }
