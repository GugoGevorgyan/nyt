...
export default {
    ...
    methods: {
        ...
        init() {
            this.map = new ymaps.Map("order-history-map-" + this.order.order_id, {
                center: [55.73, 37.75], //<- @TODO FIX THIS ROAD CENTER
                zoom: 10,
                controls: [],
            });
        ...
        setPoints() {
            if (this.stages) {
                ...
                if (
                    this.inProcess &&
                    this.board &&
                    this.board.hasOwnProperty("driver") &&
                    this.board &&
                    this.board.driver
                ) {
                    ...
                    let balloon = `
                                    <span>
                                        Водитель: ${this.board.driver_info.surname} ${this.board.driver_info.name} ${this.board.driver_info.patronymic}
                                    </span>`;
        ...
        setOrderProgress() {
            if (null !== this.orderProgress) {
                ...
                let p = new ymaps.Placemark(
                    ...
                    {
                        balloonContentBody: progress ? `<span>${this.timeFormat(progress.date)}</span>` : "",
                    },
                    {
                        iconLayout: ymaps.templateLayoutFactory.createClass(
                            this.pointHtml(
    ...
    created() {
        ymaps.ready(this.init).then(res => {
            this.setValues();
        });
    },
};
