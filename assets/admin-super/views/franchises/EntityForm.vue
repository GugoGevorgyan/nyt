...
<template>
    <div>
        <v-card :loading="loading">
            <v-card-title>Новое юридическое лицо</v-card-title>
            <v-card-text>
                <v-form :data-vv-scope="entity.scope">
                    <v-row>
                        <v-col cols="12" md="6">
                            ...
                            <div class="pa-1" style="height: 165px; border-radius: 4px; border: 1px dashed gray">
                                <div class="pa-1 " style="height: 100%; overflow-y: auto">
                                    <v-list class="pa-0" dense v-if="entity.new_banks.length">
                                        <v-list-item class="elevation-3 mb-1" v-for="(item, i) in entity.new_banks" :key="i">
                                            <v-list-item-content>
                                                <v-list-item-title class="d-flex justify-space-between align-center">
                                                    <span>{{ item.name }}</span>
                                                    <div>
                                                        <v-btn small icon color="primary" @click="updateBank(item, i)">
                                                            <v-icon small>mdi-pencil</v-icon>
                                                        </v-btn>
                                                        <v-btn small icon color="error" @click="removeBank(i)">
                            ...
                            <v-layout wrap>
                                ...
                                <v-flex xs6 class="pa-1">
                                    ...
                                    <v-menu
                                        ...
                                        v-model="registration_certificate_date_menu"
                                    >
                                        <template v-slot:activator="{ on }">
                                            <v-text-field
                                                ...
                                                label="Дата выдачи свидетельства о регистрации"
                                                readonly
                                                v-model="entity.registration_certificate_date"
                                                v-on="on"
                                                :error-messages="errors.collect(`${entity.scope}.registration_certificate_date`)"
                                                name="registration_certificate_date"
                                                v-validate="entity.rules.registration_certificate_date"
                                                data-vv-as="Дата выдачи свидетельства о регистрации"
                                            />
                                        </template>
                                        <v-date-picker
                                            @input="registration_certificate_date_menu = false"
                                            no-title
                                            v-model="entity.registration_certificate_date"
                                            :max="today()"
                                        />
                                    ...
                                    >
                                        <template v-slot:activator="{ on }">
                                            <v-text-field
                                                ...
                                                v-on="on"
                                                :error-messages="errors.collect(`${entity.scope}.tax_psrn_date`)"
                                                name="tax_psrn_date"
                                        ...
                                        <v-date-picker
                                            @input="tax_psrn_date_menu = false"
                                            no-title
                                            v-model="entity.tax_psrn_date"
                                            :max="today()"
                                        />
            ...
            <v-card-actions class="justify-end">
                <v-btn color="error" text small @click="close()">отмена</v-btn>
                <v-btn color="primary" :loading="loading" small @click="store()">принять</v-btn>
            </v-card-actions>
        </v-card>
        <v-dialog persistent v-model="bankDialog" width="600">
            <v-card :loading="bankDialogLoading">
                <v-card-title>
                    {{ ~updateBankIndex ? 'Обновить банк ' + entity.new_banks[updateBankIndex].name : 'Новый банк' }}
                </v-card-title>
                <v-card-text>
                ...
                <v-card-actions class="justify-end">
                    ...
                    <v-btn color="primary" small :loading="bankDialogLoading" @click="appendBank()">принять</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
